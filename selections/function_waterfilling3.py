...
def fgrad(x, alpha, h_i, S_i, data_size, const, later_weights,P_max):
    ...
    if isinstance(x, int) or isinstance(x, float) or (len(x) == 1 and len(h_i) == 1) :
        ...
        return S_i  *alpha / h_i / (x-const*data_size)**2 * np.exp(-alpha/h_i/(x-const*data_size))+1/P_max * later_weights
    val = (S_i  *alpha / h_i / np.square(x-const*data_size) * np.exp(-alpha/h_i/(x-const*data_size))+
           1/P_max * 10 * later_weights)
    val[x<Qth] = np.square(x[x<Qth] - Pth[x<Qth]) + const_pts[x<Qth]+ 1/P_max * later_weights[x<Qth]
    return val
...
#%% Utility
def find_maxi_fderivatives(alpha, h_i, S_i, data_size, const, later_weights,P_max):
    Pth = alpha / 2 / h_i
    Qth = Pth + const*data_size
    return (Qth, fgrad(Qth, alpha, h_i,S_i, data_size, const, later_weights,P_max))

#%%

# Given mu find corresponding P_k
# err_p = 2**(-40)
def find_Q_Given_mu(mu,  alpha, h_i, S_i,err, P_max, data_size, const, later_weights):
    Q_min, val = find_maxi_fderivatives(alpha, h_i, S_i, data_size, const, later_weights,P_max)
    ...
    fprime = fgrad(Q_k, alpha, h_i_p, S_i_p, data_size_p, const, later_weights_p, P_max)
    iter_max = 10000
    n_iter = 0
    eps = 1
...
# find mu
# err_mu = 2**(-40)
def find_mu(err_mu,err_p, alpha, h_i, S_i, P_max, P_sum,data_size, const, later_weights):
    mu_min = 1e-15
    Q_min, val =  find_maxi_fderivatives(alpha, h_i, S_i,data_size, const, later_weights,P_max)
    mu_max = np.max(val)*2
    mu = (mu_min + mu_max) /2
    Qvec = find_Q_Given_mu(mu, alpha, h_i, S_i,err_p, P_max,data_size, const, later_weights)
...
def opti_WF( K, const_alpha, h_i, S_i, P_max, P_sum,data_size, const, later_weights, hei,yo):
    err_mu = 2**(-30)
    err_p = 2**(-30)

    Q_min, val =  find_maxi_fderivatives(const_alpha, h_i, S_i,data_size, const, later_weights,P_max)
    N = len(h_i)


    Q_opt, la = find_mu(err_mu,err_p, const_alpha, h_i, S_i, P_max, P_sum,data_size, const, later_weights)
    active = (Q_opt > 1e-12)
    P_opt = np.zeros(N)
    ...
    if nb_active == 0:
        ...
        Q_opt, la = find_mu(err_mu,err_p, const_alpha, h_i[active_clients], S_uni, P_max, P_sum,
                            data_size[active_clients], const, later_weights)
        P_opt = np.zeros(N)
    ...
    while nb_active > K:
        ...
        Q_opt, la = find_mu(err_mu,err_p,  const_alpha, h_i[indvec], S_i[indvec],
                            P_max, P_sum,data_size[indvec], const, later_weights[indvec])
        active = (Q_opt > 1e-8)
        nb_active = np.sum(active)
    if nb_active < K:
        Q_opt = Q_old
